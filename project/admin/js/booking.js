      ...
      success: function(data) {
        ...
        for (let booking of data) {
          ...
          let newRow = `<tr>
            ...
            <td>${toDate}</td>
            <td>
              <button type="button" class="accept btn btn-success" onclick="updateStatusBooking(${id})">Accept</button><br><br>
...
function saveeBooking() {
   ...
   var year = posting_date.getFullYear();
   var month = (posting_date.getMonth() + 1).toString().padStart(2, '0');
   var day = posting_date.getDate().toString().padStart(2, '0');
  ...
  if (age<18) {
    swal({
      title: "Age violation",
      text: "Minimum age 18 is required!",
      icon: "error",
      button: "OK",
    }).then(() => {
      $('#cname').focus(); // Focus on the invalid field
    ...
    success: function(data) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        let newRow = '<tr>' +
          ...
          '<td>  <label> Rs :</label>' + dPrice + '<label> .00</label></td>' +
          '<td>  <label> Rs :</label>' + akmPrice + '<label> .00</label></td>' +
          '<td>  <label> Rs :' + addHourPrice + ' .00</label></td>' +
          '<td><button type="button" class="update btn btn-success" onclick="getVehicleId(' + id + ')" >Booking</button></td>' +
          '</tr>';
        $('#AvgTable tbody').append(newRow);
      }
