      ...
      success: function(data) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          let newRow = '<tr>' +
            ...
            '<td>' + description + '</td>' +
            '<td>' + date + '</td>' +
            '<td><img src="../../system/images/'+id+'/'+image+'" width="70px"></td>' +
            '<td>  <label> Rs :</label>'+ amount + '<label> .00</label></td>' +
            '<td><button type="button" class="update btn btn-success" onclick="getDamageDetails(' + id + ')" >Update</button><br><br> <button type="button" onclick="deleteEmployee(' + id + ') " class="delete btn btn-danger">Delete</button></td>'
